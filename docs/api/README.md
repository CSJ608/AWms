# API 契约导航与通用规范（已定稿 v2.1）

> 状态：**已定稿 v2.1（2026-08-10，用户确认）**。第 2 批 C-05 双向评审后锁定；第 3 批裁决修订已同步。前后端唯一依据，变更必须先改契约再改实现。
...
| 枚举与错误码.md | 全部枚举与错误码 | 🔒 锁定 v1.7 |
| 物料.md / 仓库库位.md / 来源.md | 主数据 | 🔒 锁定 v0.5 / v0.4 / v0.4；第 3 批已实现 |
| 批次.md | 主数据（系统自动建批次） | 🔒 锁定 v0.7；第 3 批已实现查询 |
| 认证权限.md | 登录、用户、角色、权限、菜单 | 🔒 锁定 v0.3；第 3 批已实现 |
| 入库单.md / 收货.md | 入库链 | 🔒 锁定 v0.3；待第 4 批实现 |
| 库存.md / 工作台.md | 库存与聚合 | 🔒 锁定 v0.3 / v0.1；待第 5 批实现 |
| 标签解析.md / 附件.md / 打印.md | 收货链支撑能力 | 🔒 锁定 v0.1 / v0.2 / v0.2；待第 4 批实现 |
| 导入导出.md | 平台导入导出能力 | 🔒 锁定 v0.2；第 3 批已实现物料模块 |
| 数据模型-草案.md | 概念级数据模型（C-02 依据） | 🔒 锁定 v0.4；按批次渐进实现 |

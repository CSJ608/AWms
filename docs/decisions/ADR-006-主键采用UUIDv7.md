...
- 分页排序必须**确定性全序**：服务端在用户/默认排序后自动追加唯一兜底（主数据 `id`；时间性列表 `createdAt, id` 且默认 createdAt DESC）。v7 主键是“近似插入序”而非严格插入序，不能替代时间列兜底。详见通用规范 2.10 v2.0 与后端工程规范 §3.2。
